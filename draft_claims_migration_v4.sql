-- ============================================================================
-- SQL MIGRATION PROPOSAL V4: FORDERUNGSMANAGEMENT (CLAIMS)
...
DECLARE
  ...
  v_payment_allocation_type text;
  v_claim_user_id uuid;
  v_item_user_id uuid;
  v_item_claim_id uuid;
BEGIN
  -- 1. Check payment exists and belongs to the correct user and claim
  SELECT user_id, claim_id, allocation_type INTO v_payment_user_id, v_payment_claim_id, v_payment_allocation_type FROM claim_payments WHERE id = NEW.claim_payment_id;
  ...
  -- Vorzeichen-Check
  IF NEW.amount > 0 AND v_payment_allocation_type = 'reversal' THEN
    RAISE EXCEPTION 'Reversal allocations must be negative or zero.';
  END IF;
  IF NEW.amount < 0 AND v_payment_allocation_type != 'reversal' THEN
    RAISE EXCEPTION 'Normal allocations must be positive or zero.';
  END IF;
...
-- Schutz von original_amount nach Zahlungszuordnung
CREATE OR REPLACE FUNCTION protect_claim_item_original_amount()
RETURNS TRIGGER
SECURITY DEFINER
SET search_path = public
AS $$
...
BEGIN
  IF NEW.original_amount != OLD.original_amount THEN
    ...
    WHERE cpa.claim_item_id = OLD.id AND cp.status = 'posted';

    IF v_posted_count > 0 THEN
      RAISE EXCEPTION 'original_amount cannot be changed after payments have been posted to this item. Use reversal or new items.';
    END IF;
  END IF;
  RETURN NEW;
END;
...
CREATE TRIGGER trigger_protect_claim_item_original_amount
BEFORE UPDATE ON claim_items
FOR EACH ROW EXECUTE PROCEDURE protect_claim_item_original_amount();
...
CREATE TABLE IF NOT EXISTS claim_payments (
  ...
  CONSTRAINT chk_reversal_amount CHECK (
    (allocation_type = 'reversal' AND amount < 0 AND reversal_reference_id IS NOT NULL) OR
    (allocation_type != 'reversal' AND amount >= 0 AND reversal_reference_id IS NULL)
  )
);
...
-- Verhindere Update auf gepostete Zahlungen
CREATE OR REPLACE FUNCTION block_posted_payment_update()
RETURNS TRIGGER 
SET search_path = public
AS $$
BEGIN
  IF OLD.status = 'posted' THEN
    RAISE EXCEPTION 'Posted payments cannot be modified. Use reversals for corrections.';
  END IF;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER trigger_block_posted_payment_update
BEFORE UPDATE ON claim_payments
FOR EACH ROW EXECUTE PROCEDURE block_posted_payment_update();
...
CREATE TABLE IF NOT EXISTS claim_payment_allocations (
  ...
  CONSTRAINT chk_bucket_item_id CHECK (
    (allocation_bucket = 'principal' AND claim_item_id IS NOT NULL) OR
    (allocation_bucket IN ('fees', 'interest') AND claim_item_id IS NULL)
  )
);
...
-- Verhindere Update auf gepostete Allocations
CREATE OR REPLACE FUNCTION block_posted_allocation_update()
RETURNS TRIGGER 
SET search_path = public
AS $$
DECLARE
  v_status text;
BEGIN
  SELECT status INTO v_status FROM claim_payments WHERE id = OLD.claim_payment_id;
  IF v_status = 'posted' THEN
    RAISE EXCEPTION 'Allocations for posted payments cannot be modified.';
  END IF;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER trigger_block_posted_allocation_update
BEFORE UPDATE ON claim_payment_allocations
FOR EACH ROW EXECUTE PROCEDURE block_posted_allocation_update();
...
BEGIN
  ...
  ELSE
    IF TG_OP = 'DELETE' THEN
      v_check_id := OLD.claim_payment_id;
    ELSE
...
WITH posted_allocations AS (
  SELECT cpa.claim_item_id, SUM(cpa.amount) as paid_principal
  FROM claim_payment_allocations cpa
  JOIN claim_payments cp ON cp.id = cpa.claim_payment_id
  WHERE cp.status = 'posted' AND cpa.allocation_bucket = 'principal'
  GROUP BY cpa.claim_item_id
)
SELECT 
  ...
  ci.user_id,
  ci.original_amount,
  COALESCE(pa.paid_principal, 0) as paid_principal,
  GREATEST(0, ci.original_amount - COALESCE(pa.paid_principal, 0)) as open_amount
FROM claim_items ci
LEFT JOIN posted_allocations pa ON pa.claim_item_id = ci.id;
...
WITH allocation_sums AS (
  ...
  JOIN claim_payment_allocations cpa ON cp.id = cpa.claim_payment_id
