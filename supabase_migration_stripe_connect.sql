-- 1. Alter profiles table to add Stripe Connect columns
ALTER TABLE public.profiles 
ADD COLUMN IF NOT EXISTS stripe_connect_id TEXT,
ADD COLUMN IF NOT EXISTS stripe_connect_enabled BOOLEAN DEFAULT false;


-- 2. Drop existing function before recreating
DROP FUNCTION IF EXISTS public.get_public_claim_by_token(text, text);


-- 3. Recreate get_public_claim_by_token including stripe_connect_enabled status
CREATE OR REPLACE FUNCTION public.get_public_claim_by_token(
    p_token text,
    p_pin text
)
RETURNS json
