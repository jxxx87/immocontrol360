-- ============================================================================
-- Migration: Smart allocation for record_claim_payment
-- ============================================================================
...
BEGIN
  ...
  SELECT created_at INTO v_active_plan_created_at
  FROM payment_plans
  ...
  -- Smart restriction for total due check:
  -- If paying an installment, we technically only care that it doesn't exceed the installment open amount.
  ...
  IF p_installment_id IS NOT NULL THEN
    ...
    v_inst_open_amount := v_inst.amount - v_inst.paid_amount;
    IF p_amount > (v_inst_open_amount + 0.01) THEN
      RAISE EXCEPTION 'Zahlungsbetrag (%.2f) ist höher als die ausgewählte offene Rate (%.2f).', p_amount, v_inst_open_amount;
    END IF;
  END IF;
  ...
  -- If paying a new item explicitly (plan active but no installment_id), bypass fees/interest for simplicity.
  -- This ensures the money goes straight to the new principal (e.g. rent) and not the old plan's fees.
  ...
  IF v_alloc_principal > 0 THEN
    ...
    -- If there is still principal to allocate (e.g. because of payment plan adjustment/Aufschlag which is not in claim_items),
    -- we must allocate it to the fees bucket to satisfy the trigger check.
    IF v_remaining_principal_to_allocate > 0 THEN
      INSERT INTO claim_payment_allocations (claim_payment_id, user_id, claim_id, allocation_bucket, amount)
      VALUES (v_payment_id, v_user_id, p_claim_id, 'fees', v_remaining_principal_to_allocate);
      v_alloc_fees := v_alloc_fees + v_remaining_principal_to_allocate;
      v_alloc_principal := v_alloc_principal - v_remaining_principal_to_allocate;
      v_remaining_principal_to_allocate := 0;
    END IF;
  END IF;

  -- 10. Process Installment if provided
  IF p_installment_id IS NOT NULL THEN
    v_inst_new_paid := v_inst.paid_amount + p_amount;
    
    IF v_inst_new_paid >= (v_inst.amount - 0.01) THEN
  ...
  IF p_installment_id IS NOT NULL THEN
    v_event_metadata := v_event_metadata || jsonb_build_object(
      ...
      'installment_due_date', v_inst.due_date,
      'installment_amount', v_inst.amount,
      'installment_paid_after', v_inst_new_paid,
      'installment_open_after', v_inst.amount - v_inst_new_paid,
      'installment_status_after', v_inst_new_status,
      'old_installment_paid_amount', v_inst.paid_amount,
      'old_installment_status', v_inst.status,
