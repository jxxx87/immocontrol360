-- Fix create_claim_advanced: original_amount = base + fee + interest per item
...
BEGIN
    ...
    -- Count total items for proportional fee/interest distribution
    v_total_items := COALESCE(array_length(p_rent_ledger_ids, 1), 0) + jsonb_array_length(p_manual_items);

    -- Calculate total open for proportional distribution
    IF p_rent_ledger_ids IS NOT NULL THEN
        FOR v_ledger IN SELECT id, lease_id, expected_rent, COALESCE(paid_amount,0) as paid_amount, period_month
            FROM rent_ledger WHERE id = ANY(p_rent_ledger_ids) AND user_id = v_user_id LOOP
            IF v_ledger.lease_id != p_lease_id THEN RAISE EXCEPTION 'Mieten müssen zum selben Vertrag gehören.'; END IF;
    ...
    -- Insert rent items: original_amount = rent + proportional fee + proportional interest
    IF p_rent_ledger_ids IS NOT NULL THEN
        FOR v_ledger IN SELECT id, expected_rent, COALESCE(paid_amount,0) as paid_amount, period_month
            FROM rent_ledger WHERE id = ANY(p_rent_ledger_ids) AND user_id = v_user_id LOOP
            v_open_amount := v_ledger.expected_rent - v_ledger.paid_amount;
            IF v_total_open_amount > 0 THEN
                v_fee_per_item := ROUND(COALESCE(p_fee_amount,0) * (v_open_amount / v_total_open_amount), 2);
                v_interest_per_item := ROUND(COALESCE(p_accumulated_interest,0) * (v_open_amount / v_total_open_amount), 2);
            ELSE v_fee_per_item := 0; v_interest_per_item := 0; END IF;
            INSERT INTO claim_items (user_id, claim_id, rent_ledger_id, item_type,
                original_amount, fee_amount, interest_amount, period_month, description, due_date)
            VALUES (v_user_id, v_new_claim_id, v_ledger.id, 'rent',
                v_open_amount + v_fee_per_item + v_interest_per_item, v_fee_per_item, v_interest_per_item,
                v_ledger.period_month, 'Mietrückstand ' || to_char(v_ledger.period_month, 'MM/YYYY'), v_ledger.period_month + 3);
        END LOOP;
    END IF;

    -- Insert manual items: original_amount = base + proportional fee + proportional interest
    FOR v_manual_item IN SELECT * FROM jsonb_array_elements(p_manual_items) LOOP
        ...
        BEGIN
            ...
            VALUES (v_user_id, v_new_claim_id, COALESCE(v_manual_item.value->>'item_type','other'),
                v_base + v_fee_per_item + v_interest_per_item, v_fee_per_item, v_interest_per_item,
