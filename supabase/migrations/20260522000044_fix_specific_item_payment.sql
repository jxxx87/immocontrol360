-- ============================================================================
-- Migration: Fix payment allocation for specific_item to include fees/interest
-- and add target_item info to timeline metadata
-- ============================================================================
...
CREATE OR REPLACE FUNCTION public.record_claim_payment(
  ...
  p_target_type text DEFAULT 'auto', -- 'auto', 'installment', 'claim_items', 'specific_item'
...
BEGIN
  ...
  -- AUTO-DETECT INSTALLMENT ONLY IF target_type != 'claim_items' AND target_type != 'specific_item'
  ...
  INTO v_total_fees_open, v_current_principal_open
  ...
  IF p_installment_id IS NOT NULL THEN
    ...
    IF NOT FOUND THEN RAISE EXCEPTION 'Rate nicht gefunden oder keine Berechtigung.'; END IF;
  ...
  -- If paying a specific item, get its info for the timeline
  ...
  -- Fees/Interest allocation: ALWAYS pay fees first, then interest, then principal
  -- This applies to ALL payment types including specific_item
  IF (v_active_plan_created_at IS NOT NULL AND p_installment_id IS NULL AND p_target_type NOT IN ('specific_item')) THEN
    -- For new items without explicit specific_item: bypass fees (already in plan)
    v_alloc_fees := 0;
    v_alloc_interest := 0;
  ELSE
    -- For auto, installment, and specific_item: pay fees first, then interest
  ...
  IF p_installment_id IS NOT NULL THEN
    ...
    FOR v_open_inst IN 
        ...
        FROM payment_plan_installments 
        WHERE payment_plan_id = v_inst.payment_plan_id AND status != 'paid' ORDER BY due_date ASC
    LOOP
  ...
  -- Calculate remaining open for the specific item after payment
  IF p_target_type = 'specific_item' AND p_target_claim_item_id IS NOT NULL THEN
  ...
  -- Add target item info if specific item payment
  IF p_target_type = 'specific_item' AND p_target_claim_item_id IS NOT NULL THEN
    v_event_metadata := v_event_metadata || jsonb_build_object(
      'target_type', 'specific_item',
      'target_item_id', p_target_claim_item_id,
      'target_item_description', v_target_item_description,
      'target_item_open_before', v_target_item_open_before,
      'target_item_open_after', v_target_item_open_after
    );
  END IF;
