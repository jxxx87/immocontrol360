...
BEGIN
  ...
  -- 3. Load Claim
  SELECT status, interest_rate, interest_start_date, accumulated_unpaid_interest
  INTO v_claim_status, v_interest_rate, v_interest_start_date, v_accumulated_unpaid_interest
  FROM claims
  ...
  -- Get fees and principal from totals view
  SELECT 
  ...
  -- Add a small epsilon (0.01) to prevent floating point/numeric rounding rejections
  ...
  IF v_alloc_principal > 0 THEN
    ...
    -- If there's still money left over and no items, it shouldn't happen because of step 5, but just in case
    IF v_remaining_principal_to_allocate > 0.01 THEN
       RAISE EXCEPTION 'Zahlungsbetrag kann nicht auf offene Positionen verteilt werden (Rundungsfehler oder inkonsistenter Zustand).';
    END IF;
  END IF;
