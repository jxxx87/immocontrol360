-- Fix create_claim_advanced description and encoding
...
BEGIN
    ...
    IF p_rent_ledger_ids IS NOT NULL AND array_length(p_rent_ledger_ids, 1) > 0 THEN
        ...
        IF array_length(p_rent_ledger_ids, 1) = 1 THEN
            v_desc_rent := 'Mietrückstand ' || substring(v_desc_rent from 1 for length(v_desc_rent) - 2);
        ELSE
            v_desc_rent := 'Mietrückstände ' || substring(v_desc_rent from 1 for length(v_desc_rent) - 2);
        END IF;
