-- Recreate record_claim_payment to include all missing rollback metadata (interest_start_date, installment status)
...
BEGIN
  ...
  -- Build event metadata with critical fallback fields for reverse_claim_payment
...
-- Recreate reverse_claim_payment with resilient status reset behavior
