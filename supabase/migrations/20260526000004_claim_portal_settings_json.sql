...
ADD COLUMN IF NOT EXISTS claim_portal_allow_installments BOOLEAN DEFAULT true,
ADD COLUMN IF NOT EXISTS claim_portal_installment_options JSONB DEFAULT '[{"months": 3, "surcharge_percent": 7}, {"months": 6, "surcharge_percent": 9}, {"months": 12, "surcharge_percent": 12}]'::jsonb;

-- Update the RPC to use the new settings JSONB
...
DECLARE
    ...
    v_result JSON;
BEGIN
    -- Hash token and pin to match with the database
