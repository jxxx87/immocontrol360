-- ============================================================================
-- Migration: Add p_target_type to skip auto-detect for payment plans
-- ============================================================================
...
CREATE OR REPLACE FUNCTION public.record_claim_payment(
  ...
  p_target_type text DEFAULT 'auto' -- 'auto', 'installment', 'claim_items'
)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- AUTO-DETECT INSTALLMENT ONLY IF target_type != 'claim_items'
  IF v_active_plan_id IS NOT NULL AND p_installment_id IS NULL AND p_target_type != 'claim_items' THEN
