-- ============================================================================
-- Migration: Support paying specific claim item
-- ============================================================================
...
BEGIN
  ...
  -- If paying a specific item or an installment where fees are waived by the plan, we might handle fees differently.
  -- But usually payments pay fees/interest first.
  -- Wait! If target_type = 'specific_item', we might WANT to bypass fees entirely and only pay the principal of that item!
  -- Let's bypass fees/interest if they specifically chose a new item, UNLESS they overpay?
  -- Let's stick to the standard: pay fees/interest first, then principal.
  IF (v_active_plan_created_at IS NOT NULL AND p_installment_id IS NULL AND p_target_type != 'specific_item') THEN
    -- Wait, if they pay new items generally, we bypass fees? Actually fees belong to the claim.
    v_alloc_fees := 0;
    v_alloc_interest := 0;
  ELSIF p_target_type = 'specific_item' THEN
    -- Bypass fees to directly pay the selected item!
    v_alloc_fees := 0;
    v_alloc_interest := 0;
  ELSE
