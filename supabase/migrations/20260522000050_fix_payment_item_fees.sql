...
DECLARE
  v_user_id uuid; v_lease_id uuid; v_claim_status text;
  v_interest_rate numeric; v_interest_start_date date; v_accumulated_unpaid_interest numeric;
  v_total_fees_open numeric; v_total_interest_open numeric; v_current_principal_open numeric; v_total_due numeric;
  v_new_interest numeric; v_remaining_amount numeric;
  v_alloc_fees numeric := 0; v_alloc_interest numeric := 0; v_alloc_principal numeric := 0;
  v_remaining_principal_to_allocate numeric := 0;
  v_payment_id uuid; v_item record; v_item_alloc numeric;
  v_inst record; v_open_inst record; v_inst_open_amount numeric; v_inst_alloc numeric;
  v_inst_new_paid numeric; v_inst_new_status text; v_all_installments_paid boolean;
  v_remaining_installment_amount numeric; v_event_metadata jsonb;
  v_active_plan_id uuid := NULL; v_active_plan_created_at timestamptz := NULL;
  v_modified_installments jsonb := '[]'::jsonb; v_inserted_sum numeric := 0;
  v_target_item_description text := NULL; v_target_item_open_before numeric := 0;
  v_target_item_open_after numeric := 0;
  v_item_fee numeric := 0; v_item_interest numeric := 0;
  v_item_fee_already_paid numeric := 0; v_item_interest_already_paid numeric := 0;
  v_item_fee_open numeric := 0; v_item_interest_open numeric := 0;
BEGIN
  v_user_id := auth.uid();
  IF v_user_id IS NULL THEN RAISE EXCEPTION 'Not authenticated'; END IF;
  IF p_amount <= 0 THEN RAISE EXCEPTION 'Betrag muss > 0 sein.'; END IF;
  ...
  IF p_amount > (v_total_due + 0.01) THEN
    RAISE EXCEPTION 'Betrag (%.2f) > offene Forderung (%.2f).', p_amount, v_total_due; END IF;
  ...
  IF p_installment_id IS NOT NULL THEN
    ...
    IF v_inst.plan_claim_id != p_claim_id THEN RAISE EXCEPTION 'Rate gehört nicht hierher.'; END IF;
  END IF;
  ...
  IF v_alloc_fees > 0 THEN
    ...
    VALUES (v_payment_id, v_user_id, p_claim_id, 'fees', v_alloc_fees); END IF;
  IF v_alloc_interest > 0 THEN
    INSERT INTO claim_payment_allocations (claim_payment_id, user_id, claim_id, allocation_bucket, amount)
    VALUES (v_payment_id, v_user_id, p_claim_id, 'interest', v_alloc_interest); END IF;
  ...
  IF p_installment_id IS NOT NULL THEN
    ...
    FOR v_open_inst IN SELECT id, due_date, amount, paid_amount, status
        ...
        IF v_inst_new_paid >= (v_open_inst.amount - 0.01) THEN v_inst_new_status := 'paid'; ELSE v_inst_new_status := 'partial'; END IF;
  ...
  UPDATE claims SET accumulated_unpaid_interest = GREATEST(0, v_total_interest_open - v_alloc_interest),
    interest_start_date = p_payment_date,
    status = CASE WHEN p_amount >= (v_total_due - 0.01) THEN 'settled' ELSE status END, updated_at = now()
  WHERE id = p_claim_id;

  -- Remaining for specific item
  IF p_target_type = 'specific_item' AND p_target_claim_item_id IS NOT NULL THEN
    v_target_item_open_after := GREATEST(0, v_target_item_open_before - v_alloc_principal);
  END IF;
